...
class Logger(object):
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            # 找到验证值最高的那些行的索引
            valid_max = result[:, 1].max()
            valid_indices = (result[:, 1] == valid_max).nonzero(as_tuple=False).squeeze()

            # 在这些行里面选择测试值最高的
            if valid_indices.ndimension() == 0:  # 如果只有一个索引
                argmax = valid_indices.item()
            else:
                test_values = result[valid_indices, 2]
                argmax_in_valid_indices = test_values.argmax().item()
                argmax = valid_indices[argmax_in_valid_indices].item()
